class Admin::FloorPlansController < ApplicationController
  layout "admin"
  before_action :set_floor_plan, only: [:show, :edit, :update, :destroy, :download, :pdownload]
  ...
  def download
	...
	def pdownload
    response.headers['Content-Type'] = @floor_plan.pplan_content_type
    response.headers['Content-Disposition'] = 'attachment; filename=' + @floor_plan.pplan_file_name
    send_file @floor_plan.pplan.path
  end
  ...
  private
    ...
    def floor_plan_params
      params.require(:floor_plan).permit(:residence, :price, :bedrooms, :bathrooms, :balcony, :indoor_sq_ft, :indoor_sq_m, :outdoor_sq_ft, :outdoor_sq_m, :common_charges, :monthly_total, :pilot, :avaliable_general, :avaliable_admins, :plan, :pplan, :sequence, :additions => [])
    end
end
